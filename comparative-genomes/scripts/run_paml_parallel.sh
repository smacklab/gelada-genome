...
if [ $end -gt $(wc -l id_lists/orthogroups/sco_most.txt | cut -d ' ' -f 1) ]; then
	end=$(wc -l id_lists/orthogroups/sco_most.txt | cut -d ' ' -f 1)
fi
...
$parallel "$srun scripts/run_paml_single.sh {1} > logs/parallel.${SLURM_JOB_ID}_{1}.log" ::: $(eval echo -e {${start}..${end}})
